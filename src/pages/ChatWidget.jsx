  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch('https://walrus.kalavishva.com/webhook/convox_trial', {
        ...
        body: JSON.stringify({ user_id: userId, session_id: sessionId, timestamp, user_message: userMessage }),
      });
    ...
    <div>
      ...
      {isOpen && (
        <div
          ...
          style={{ backgroundColor: 'white', borderColor: primaryColor, fontFamily }}
        >
          ...
          <div className="p-2 bg-[var(--secondaryColor)] flex items-center gap-2 border-t border-gray-300">
            <input
              ...
              className="flex-1 px-4 py-2 rounded-full border bg-white border-gray-300 focus:outline-none text-sm"
              style={{ fontFamily }}
            />
            <button
              onClick={handleSend}
